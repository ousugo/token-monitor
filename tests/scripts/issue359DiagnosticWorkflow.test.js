...
test('issue 359 diagnostic workflow changes only the Electron comparison version', () => {
  assert.match(workflow, /variant: electron-43\.4\.0\s+electron: 43\.4\.0\s+koffi: 3\.1\.4/);
...
test('issue 359 diagnostic workflow signs but never publishes its artifacts', () => {
  ...
  assert.match(workflow, /name: issue-359-\$\{\{ matrix\.variant \}\}-koffi-\$\{\{ matrix\.koffi \}\}/);
